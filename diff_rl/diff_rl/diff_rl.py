...
from diff_rl.diff_rl.policies import Consistency_Actor, MlpPolicy, TD3Policy
...
class TD3(OffPolicyAlgorithm):
    ...
    def train(self, gradient_steps: int, batch_size: int = 100):
        ...
        for iter in range(gradient_steps):
            self._n_updates += 1
            ...
            with th.no_grad():
                ...
                # current_q_values = list(self.critic(replay_data.observations, replay_data.actions))
                # for i in range(len(current_q_values)):
                #     current_q_values[i] = current_q_values[i] + 0.05 * target_q_values
            
            # TODO, need to modify the loss here
            # Also, here we have two critic, how to we deal with it
            ...
            if self._n_updates % self.policy_delay == 0:
                ...
                bc_loss = self.actor.consistency_loss(iter, gradient_steps, replay_data.observations, replay_data.actions, target_model=self.actor_target)
